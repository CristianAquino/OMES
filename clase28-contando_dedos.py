    ...
    if ret == True:
        ...
        if bg is not None:
            ...
            cv2.rectangle(frame, (380-2, 50+2), (600+2, 300+2),
                          color_fingers, 1)  # aparece cada vez q pongo i
            ...
            # ordenar de mayor a menor segun el area y obtener el mayor
            ...
            for c in cnt:
                ...
                if defects is not None:
                    ...
                    for i in range(defects.shape[0]):  # desenpaquetar puntos
                        ...
                        # ayuda a calcular la distancia entre dos puntos
                        if np.linalg.norm(start-end) > 20 and d > 12000:
                            inicio.append(start)
                            fin.append(end)
                            # punto inicial(turquesa)
                            cv2.circle(ROI, tuple(start), 5, color_start, 2)
                            # punto final(morado)
                            cv2.circle(ROI, tuple(end), 5, color_end, 2)
                            # punto mas alejado(azul)
                            cv2.circle(ROI, tuple(far), 7, color_far, -1)
                    if len(inicio) == 0:
                        minY = np.linalg.norm(ymin[0]-[x, y])
                        if minY >= 110:
                            finger = finger + 1
                            cv2.putText(ROI, f'{finger}', tuple(
                                ymin[0]), 1, 1, color_fingers, 1, cv2.LINE_AA)

                    # es inicio xq hay la misma cantidad al inicio y final
                    for i in range(len(inicio)):
                        finger = finger + 1
                        cv2.putText(ROI, f'{finger}', tuple(
                            inicio[i]), 1, 1, color_fingers, 1, cv2.LINE_AA)
                        if i == len(inicio)-1:
                            finger = finger + 1
                            cv2.putText(ROI, f'{finger}', tuple(
                                fin[i]), 1, 1, color_fingers, 1, cv2.LINE_AA)

                    cv2.putText(frame, f'{finger}', (390, 45),
                                1, 1, color_fingers, 2, cv2.LINE_AA)
