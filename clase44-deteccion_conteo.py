...
video = cv2.VideoCapture(get('video3'))
...
while True:
    ...
    area_pts = np.array([[330, 216], [frame.shape[1]-80, 216],
                        [frame.shape[1]-80, 271], [330, 271]])
    ...
    cv2.rectangle(frame, (frame.shape[1]-70, 215),
                  (frame.shape[1]-5, 270), (0, 255, 0), 2)
    cv2.putText(frame, str(car_counter),
                (frame.shape[1]-55, 250), 0, 1.2, (0, 255, 0), 2)
    cv2.imshow('A', frame)
    #cv2.imshow('B', fgmask)
