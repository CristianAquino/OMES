...
cntAzul, _ = cv2.findContours(
    maskAzul, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
dibujar_contorno(cntAzul, (255, 0, 0))

cntAmarillo, _ = cv2.findContours(
    maskAmarillo, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
dibujar_contorno(cntAmarillo, (0, 255, 255))

cntVerde, _ = cv2.findContours(
    maskVerde, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
dibujar_contorno(cntVerde, (0, 255, 0))

cntNaranja, _ = cv2.findContours(
    maskNaranja, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
dibujar_contorno(cntNaranja, (17, 70, 244))

cntRojo, _ = cv2.findContours(
    maskRojo, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
